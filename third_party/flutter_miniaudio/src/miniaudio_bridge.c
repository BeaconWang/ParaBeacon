...
// Portable full memory barrier.
// __sync_synchronize() is a GCC/Clang builtin and is NOT provided by MSVC,
// which causes LNK2019 (unresolved external symbol) on Windows. Map to the
// appropriate intrinsic per compiler.
#if defined(_MSC_VER)
  #include <intrin.h>
  // _ReadWriteBarrier(): compiler barrier; _mm_mfence(): full hardware fence.
  // Both come from <intrin.h>, so we avoid pulling in <windows.h>.
  #define MA_BRIDGE_MEMORY_BARRIER() do { _ReadWriteBarrier(); _mm_mfence(); } while (0)
#else
  #define MA_BRIDGE_MEMORY_BARRIER() __sync_synchronize()
#endif
